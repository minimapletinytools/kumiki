...
def cut_rebated_oblique_and_dadoed_scarf_joint_on_aligned_timbers(
    ...
    rebated_shoulder_depth: Numeric,
    scarf_length: Numeric,
    dado_depth: Numeric,
    dado_height: Numeric,
    stub_tenon_width: Numeric,
    rebated_shoulder_length: Optional[Numeric] = None,
    joint_center_relative_to_timber1_end: Numeric = scalar(0),
    lateral_offset_from_midline: Numeric = scalar(0),
) -> Joint:
    """
    Creates a rebated oblique and dadoed scarf joint (金輪継ぎ / Kanawa Tsugi).
    ...
    Args:
        ...
        rebated_shoulder_depth: (Tsuki-tsuke (突付)) determines the depth of the rebated shoulder cut in the scarf joint, the 2 rebated shoulders form the pin hole for the square peg forming the Kusabi-ana (楔穴)
        ...
        rebated_shoulder_length: determines the length of the rebated shoulder cut in the scarf joint, if None, defaults to rebated_shoulder_depth (forming a rectangular peg hole). Note this measures the width of the rectangular hole that gets created by the rebated shoulder, rather than the width from corner to corner of the rectangular hole which is how you might draw this joint by hand.
    ...
    Notes:
        the oblique scarf face angle (Sogi-michi (斜面)) is determined by the rebated_shoulder_depth and the scarf_half_length
    ...
    if rebated_shoulder_length is None:
        rebated_shoulder_length = rebated_shoulder_depth

    SL = scarf_length
    DD = dado_depth
    DH = dado_height
    SSD = rebated_shoulder_depth
    SSL = rebated_shoulder_length
    STW = stub_tenon_width

    require_check(
        None if SSL == SSD
        else "rebated_shoulder_length must equal rebated_shoulder_depth: the Kusabi peg accessory "
    ...
    require_check(None if SL >= SSD else "rebated_shoulder_depth must be less than scarf_length")
    require_check(None if DD > 0 else "dado_depth must be positive")
    require_check(None if DH > 0 else "dado_height must be positive")
    require_check(None if SSD > 0 else "rebated_shoulder_depth must be positive")
    ...
    require_check(None if DD < (SL - SSD) / scalar(2) else "dado_depth must be less than (scarf_length - rebated_shoulder_depth) / 2")
    require_check(None if DH >= SSD else "dado_height must be at least rebated_shoulder_depth")
...
def cut_half_blind_tenoned_dadoed_rabbeted_scarf_joint_on_aligned_timbers(
    arrangement: SpliceJointTimberArrangement,
    stepped_shoulder_depth: Optional[Numeric] = None,
    scarf_length: Optional[Numeric] = None,
    dado_depth: Optional[Numeric] = None,
    dado_height: Optional[Numeric] = None,
    stub_tenon_width: Optional[Numeric] = None,
    stepped_shoulder_length: Optional[Numeric] = None,
    joint_center_relative_to_timber1_end: Numeric = scalar(0),
    lateral_offset_from_midline: Numeric = scalar(0),
    *,
    rebated_shoulder_depth: Optional[Numeric] = None,
    rebated_shoulder_length: Optional[Numeric] = None,
) -> Joint:
    """Compatibility alias for cut_rebated_oblique_and_dadoed_scarf_joint_on_aligned_timbers."""
    actual_depth = rebated_shoulder_depth if rebated_shoulder_depth is not None else stepped_shoulder_depth
    actual_length = rebated_shoulder_length if rebated_shoulder_length is not None else stepped_shoulder_length
    assert actual_depth is not None, "rebated_shoulder_depth (or stepped_shoulder_depth) must be provided"
    assert scarf_length is not None, "scarf_length must be provided"
    assert dado_depth is not None, "dado_depth must be provided"
    assert dado_height is not None, "dado_height must be provided"
    assert stub_tenon_width is not None, "stub_tenon_width must be provided"
    return cut_rebated_oblique_and_dadoed_scarf_joint_on_aligned_timbers(
        arrangement=arrangement,
        rebated_shoulder_depth=actual_depth,
        scarf_length=scarf_length,
        dado_depth=dado_depth,
        dado_height=dado_height,
        stub_tenon_width=stub_tenon_width,
        rebated_shoulder_length=actual_length,
        joint_center_relative_to_timber1_end=joint_center_relative_to_timber1_end,
        lateral_offset_from_midline=lateral_offset_from_midline,
    )


# Aliases for Japanese joint functions
cut_kanawa_tsugi_joint_on_aligned_timbers = cut_rebated_oblique_and_dadoed_scarf_joint_on_aligned_timbers
cut_kanawa_tsugi = cut_rebated_oblique_and_dadoed_scarf_joint_on_aligned_timbers
cut_金輪継ぎ_joint_on_aligned_timbers = cut_rebated_oblique_and_dadoed_scarf_joint_on_aligned_timbers
cut_金輪継ぎ = cut_rebated_oblique_and_dadoed_scarf_joint_on_aligned_timbers
cut_かなわつぎ_joint_on_aligned_timbers = cut_rebated_oblique_and_dadoed_scarf_joint_on_aligned_timbers
cut_かなわつぎ = cut_rebated_oblique_and_dadoed_scarf_joint_on_aligned_timbers
