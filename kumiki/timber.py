...
@dataclass(frozen=True)
class Measure:
    """A dimension between two features, in one of a drawing's viewports.

    The anchors are feature paths, the same names the viewer picks by. What the
    dimension means is decided where it is drawn: a drawing is a projection, so
    the two anchors are projected onto that viewport's plane and measured there.
    The same pair measured in two viewports is two dimensions with two numbers,
    which is why the viewport is named here.

    Identity is the anchors, plus `measure_id` when the same pair is measured
    more than once -- deliberately not a position in a list, so that a
    measurement generated by an algorithm keeps whatever the file has said about
    it when the algorithm next runs and emits a different number of them.

    The viewport is a property rather than part of that identity, so a
    measurement moved to another viewport is still the same measurement.
    """
    ...
    def identity(self) -> Tuple[str, str, str]:
        """What makes this measurement itself, wherever it is drawn.
...
class Drawing:
    ...
    # Dimensions the frame asks for. Written out by hand today; expected to be
    # mostly generated by algorithm later, which is what the identity rules on
    # Measure are for.
