...
class TestIntersectionNode:
    """Tests for the Intersection CSG node."""
...
class TestConvexPolygonSimpleLoft:
    ...
    def test_constructor(self):
        ...
        loft = ConvexPolygonSimpleLoft(bottom_points=bottom, top_points=top,
                                  bottom_points_z_pos=scalar(0), top_points_z_pos=scalar(10),
                                  transform=Transform.identity())

        assert loft.bottom_points == bottom
        assert loft.top_points == top
        assert loft.bottom_points_z_pos == scalar(0)
        assert loft.top_points_z_pos == scalar(10)
    ...
    def test_is_valid_requires_end_after_start(self):
        ...
        loft_zero = ConvexPolygonSimpleLoft(bottom_points=bottom, top_points=top,
                                       bottom_points_z_pos=scalar(5), top_points_z_pos=scalar(5))
        assert loft_zero.is_valid() == False

        loft_negative = ConvexPolygonSimpleLoft(bottom_points=bottom, top_points=top,
                                           bottom_points_z_pos=scalar(5), top_points_z_pos=scalar(0))
    ...
    def test_contains_point_caps(self):
        ...
        top = self._square(scalar(1))
        loft = ConvexPolygonSimpleLoft(bottom_points=bottom, top_points=top,
                                  bottom_points_z_pos=scalar(0), top_points_z_pos=scalar(4))
    ...
    def test_get_aabb(self):
        ...
        top = self._square(scalar(1))
        loft = ConvexPolygonSimpleLoft(bottom_points=bottom, top_points=top,
                                  bottom_points_z_pos=scalar(0), top_points_z_pos=scalar(5),
                                  transform=Transform.identity())
    ...
    def test_adopt_csg(self):
        ...
        for axis in range(3):
            assert safe_equality_test(adopted.transform.position[axis], orig_transform.position[axis])
...
class TestGetAABB:
    ...
    def test_bbox_difference_halfspace_crop_non_orthogonal(self):
        ...
        assert safe_zero_test(bbox.max_x - 5 * sqrt(scalar(2)))
        assert safe_zero_test(bbox.max_y - 5 * sqrt(scalar(2)))

    # ------------------------------------------------------------------
    # Empty CSG propagation
    # ------------------------------------------------------------------

    def test_bbox_empty_csg_is_flagged_empty(self):
        """EmptyCSG's bbox is flagged empty, not a real zero-size box at the origin."""
        bbox = EmptyCSG().get_aabb()
        assert bbox.is_empty

    def test_bbox_union_ignores_empty_children(self):
    ...
    def test_bbox_union_of_only_empty_children_is_empty(self):
    ...
    def test_bbox_intersection_with_empty_is_empty(self):
    ...
    def test_bbox_difference_of_empty_base_is_empty(self):
    ...
    def test_bbox_difference_fully_consumed_is_empty(self):
...
class TestPointQueryTolerance:
    ...
    def test_the_default_feature_tolerance_is_picking_shaped(self):
        """Feature queries exist for picking, so the default is generous.

        A raycast hit lands on a mesh vertex, not the analytic surface; at
        EPSILON_GENERIC almost none would register.
        """
        prism = self._prism()
        near_miss = create_v3(scalar(2) + 1e-6, scalar(0), scalar(5))
...
class TestFeatureLocate:
    ...
    def _taper(self):
        """A pure taper: every side flat, which is what joinery actually cuts."""
        return ConvexPolygonSimpleLoft(
    ...
    def test_a_lofted_side_locates_now_that_it_cannot_twist(self):
        """It used to decline whatever the shape, because a twist COULD make it
        a ruled surface. is_valid refuses a twisted loft, so a side that exists
    ...
    def test_and_that_plane_contains_the_corners_of_its_side(self):
        ...
        corners = [Matrix([scalar(-2), scalar(-2), scalar(0)]),
                   Matrix([scalar(2), scalar(-2), scalar(0)]),
                   Matrix([scalar(1), scalar(-1), scalar(10)]),
                   Matrix([scalar(-1), scalar(-1), scalar(10)])]
        for corner in corners:
            off = float(safe_dot_product(plane.normal, corner - plane.point))
...
class TestNonRealFeatures:
    ...
    def test_the_edge_tolerance_is_what_widens_an_axis(self):
        """A centre line is an EDGE, so the edge tolerance governs it."""
        prism = RectangularPrism(
            size=Matrix([scalar(4), scalar(6)]),
            transform=Transform.identity(),
            start_distance=scalar(0),
...
class TestDegeneracyGuardsIgnoreQueryTolerance:
    """Whether an edge is zero-length is a property of the shape, not of the query.

    Regression: the eps threading initially fed the query tolerance into the
    degeneracy guards, which compare a SQUARED edge length. A pick tolerance of
    5e-4 then declared any edge shorter than ~22mm to be zero-length, so real
    faces of small parts silently stopped resolving. Caught only because the
    default tolerance later moved from 1e-8 to a picking-shaped value.
    """

    def _small_extrusion(self):
        """A 20mm square -- edge_length_sq is 4e-4, below a 5e-4 pick eps."""
        side = scalar(2, 100)
        return ConvexPolygonExtrusion(
            points=[
                Matrix([scalar(0), scalar(0)]),
                Matrix([side, scalar(0)]),
                Matrix([side, side]),
                Matrix([scalar(0), side]),
            ],
            transform=Transform.identity(),
            start_distance=scalar(0),
...
class TestFeatureTestTolerancesScaling:
    ...
    def test_scaling_actually_changes_what_resolves(self):
        prism = RectangularPrism(
            size=Matrix([scalar(4), scalar(6)]),
            transform=Transform.identity(),
            start_distance=scalar(0),
...
class TestPlaneIntersection:
    ...
    def test_two_perpendicular_planes_meet_in_a_line(self):
        x2 = self._plane((scalar(1), scalar(0), scalar(0)), (scalar(2), scalar(0), scalar(0)))
        y3 = self._plane((scalar(0), scalar(1), scalar(0)), (scalar(0), scalar(3), scalar(0)))
        line = intersect_planes(x2, y3)
        assert line is not None
        assert float(line.point[0]) == pytest.approx(2.0)
        assert float(line.point[1]) == pytest.approx(3.0)
        assert abs(float(line.direction[2])) == pytest.approx(1.0)

    def test_parallel_planes_never_meet(self):
        x2 = self._plane((scalar(1), scalar(0), scalar(0)), (scalar(2), scalar(0), scalar(0)))
        x5 = self._plane((scalar(1), scalar(0), scalar(0)), (scalar(5), scalar(0), scalar(0)))
        assert intersect_planes(x2, x5) is None
        assert planes_are_parallel(x2, x5)

    def test_coincident_planes_share_a_plane_not_a_line(self):
        x2 = self._plane((scalar(1), scalar(0), scalar(0)), (scalar(2), scalar(0), scalar(0)))
        assert intersect_planes(x2, x2) is None

    def test_a_missing_plane_yields_nothing(self):
        """locate() declines for curved surfaces; that propagates."""
        x2 = self._plane((scalar(1), scalar(0), scalar(0)), (scalar(2), scalar(0), scalar(0)))
        assert intersect_planes(x2, None) is None
        assert intersect_planes(None, x2) is None
...
class TestDerivedEdges:
    ...
    def _owned(self, prism, name):
        ...
        return OwnedFeatureHit(feature=feature, owner=prism)
    ...
    def test_opposite_faces_derive_nothing(self):
        """Parallel planes never meet, so there is no edge to name."""
        prism = self._prism(self._face("right", PrismFace.RIGHT),
                            self._face("left", PrismFace.LEFT))
...
class TestDerivedEdgesInAQuery:
    ...
    def test_a_primitive_derives_its_own_arrises(self):
        ...
        This is the path kigumi picking takes today -- it navigates to a leaf
        primitive and queries that -- so it is what makes edges reachable from
        the viewer at all.
        """
        prism = RectangularPrism(
            size=Matrix([scalar(4), scalar(6)]),
...
class TestGatherRefineIsolation:
    """One wide gather, then per-purpose refinement -- and the refinements
    must not leak into each other.

    Regression: an earlier version derived edges straight from the wide gather
    without refining to the edge tolerance first, so raising an unrelated
    tolerance (point) silently widened the edge snap.
    """
    ...
    def test_the_point_tolerance_does_not_widen_the_edge_snap(self):
        """The gather runs at the widest tolerance; only refinement decides."""
        csg = self._cut_timber()
        gap = scalar(3, 1000)
...
class TestBuriedFacesAreNotReported:
    """A face swallowed by a sibling is surface the solid does not have.

    Regression: SolidUnion gathered its children's hits without re-gating on
    its own boundary, so a small member's face buried inside a larger one was
    reported -- and, once edges arrived, derived a spurious arris there too.
    Difference and Intersection already gated; the union did not.
    """
    ...
    def _swallowing(self):
        return RectangularPrism(
            size=Matrix([scalar(20), scalar(20)]),
            transform=Transform.identity(),
            start_distance=scalar(0),
            end_distance=scalar(10),
        )

    def test_a_face_inside_a_sibling_is_not_reported(self):
...
class TestALoftMayNotTwist:
    ...
    against the other and it becomes a saddle: no single normal, no plane to
    locate, and get_outward_normal reduced to parametric derivatives.

    Refused rather than supported, because a taper that scales each axis
    independently -- which is what joinery cuts -- stays flat anyway. Twisting
    was never reachable from a joint; it was reachable from this constructor.
    """

    def _loft(self, bottom, top):
        return ConvexPolygonSimpleLoft(
            bottom_points=[Matrix([scalar(a), scalar(b)]) for a, b in bottom],
            top_points=[Matrix([scalar(a), scalar(b)]) for a, b in top],
            bottom_points_z_pos=scalar(0),
            top_points_z_pos=scalar(10),
            transform=Transform.identity(),
        )
    ...
    def test_a_prism_is_valid(self):
        assert self._loft(self.SQUARE, self.SQUARE).is_valid()

    def test_a_taper_scaled_on_both_axes_is_valid(self):
        assert self._loft(self.SQUARE, [(1, 1), (-1, 1), (-1, -1), (1, -1)]).is_valid()

    def test_a_taper_on_one_axis_only_is_valid(self):
        # The shape a relief pocket takes: narrower at the top, same width.
        assert self._loft(self.SQUARE, [(2, 1), (-2, 1), (-2, -1), (2, -1)]).is_valid()

    def test_a_taper_that_leans_sideways_is_valid(self):
        # Still flat: every side stays a plane when the top slides across.
        assert self._loft(self.SQUARE, [(3, 1), (-1, 1), (-1, -1), (3, -1)]).is_valid()

    def test_the_same_square_turned_a_quarter_is_refused(self):
        # Identical profiles, so every earlier check passes -- convex, matching
        # counts, matching winding. Only the correspondence is twisted.
        assert not self._loft(self.SQUARE, [(1, 1), (1, -1), (-1, -1), (-1, 1)]).is_valid()

    def test_and_so_is_a_slight_turn(self):
        assert not self._loft(
            self.SQUARE,
            [(1.2, 0.8), (-0.8, 1.2), (-1.2, -0.8), (0.8, -1.2)]).is_valid()

    def test_a_valid_loft_has_a_plane_for_every_side(self):
