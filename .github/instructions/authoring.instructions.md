...
### The Joint/Timber Feature tree (TODO rewrite this section so it's not vibe coded and less wordy...)

CutTimbers contain Joints which contain Cuttings which contain a feature tree
These all get assembled into CSG feature tree that can be navigated in the kigumi viewer.

A feature tree has several tiers of labels

1. the timber itself
2. the joint
3. a CSG in the joint
4. a feature on the CSG primitive

the timber is held in a CutTimebr object which contains all Cuttings from joints. Each cutting produces a SolidUnion CSG object labeled by the name of the joint.
CSGs within the jiont are labeled by the joint cutting function.

Give structurally descriptive labels for everything based on the rules below. Not all CSG nodes need to be labeled.

What the viewer shows for a node is its **kind** and its **label** together --
`prism · tenon`, `union · mortise_and_tenon` -- and above it, the joint that made the
cut. So:

- **Do not put the kind in the label.** The label for a `RectangularPrism` is `tenon`,
  not `tenon_prism`. The kind is already displayed beside it.
- **Do not put the joint name in the label.** A joint's identity comes from its
  `JointTicket` and is shown separately. Name the cut mechanically instead:
  `butt_cross_cut`, `miter_cut`, `lap_cut` -- not `plain_butt`.
- **snake_case**, matching the rest of the library.

A `Difference` that represents material coming away is named `<thing>_waste` --
`tenon_waste` is the shoulder half-space minus the tenon, i.e. everything removed
around the tenon. A half-space that bounds a cut by depth is `<thing>_depth_plane`.

**Never nest a label inside itself.** Two nodes with the same label on one path
produce `lap_cut/lap_cut`, which reads as two levels and navigates ambiguously. If a
shavings helper already names the geometry, the cutting around it needs a different
name (`splice_lap_cut` around the helper's `lap_cut`). When a node accumulates more
removed material -- relief, peg holes, clearance -- extend the existing union rather
than wrapping it in a second one with the same label.

`top_end_cut` and `bottom_end_cut` are **reserved**. They are generated by a Cutting's
own `maybe_top/bottom_end_cut_distance_from_bottom`, and using them elsewhere makes an
ordinary plane read as an end cut. A cutting may describe the same plane twice, once as
its `negative_csg` and once as end-cut metadata; both are kept, and a search finds the
first, so the cut the joint authored is the one that answers for the plane.

Helpers in `shavings/` that build geometry take a `label: CutCSGLabel` parameter with a
default naming the kind of cut they make (`lap_cut`, `profile_cut`,
`timber_end_prism_cut`), so callers need not pass one but can say what the cut is for in
their own terms. When several pieces make one cut, name the union and leave the pieces
unnamed rather than repeating the name on each.

Labels are navigable paths, so try not to mess with them too much as they may break things in the future, however breaking labels is still allowed in a patch version bump.
